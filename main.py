...
def requests_get(url,timeout=30):
    try:
        ...
        with urllib.request.urlopen(url.replace(" ","%20"), timeout=timeout) as response:
...
class ArxivStorePlugin(BasicStoreConfig, StorePlugin):
    ...
    def search(self, query:str, max_results=10, timeout = 10) -> List[Dict]:
        ...
        url = (
            "http://export.arxiv.org/api/query?search_query=ti:{}".format(query) 
    ...
    def _build_search_result(self, result: Dict[str, Any]) -> SearchResult:
        try:
            ...
            if isinstance(result['author'], dict):
                ...
                search_res.author = ", ".join([author['name'] for author in result['author']])
    ...
    @staticmethod
    def _get_link(links:List[Dict], pdf=True) -> str:
        for link in links:
